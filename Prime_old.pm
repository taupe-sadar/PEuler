...
sub next_prime
{
	while($index>$#prime_list)
	{
		find_next_prime();
	}
	my($ret)=$prime_list[$index];
	$index++;
	return $ret;
}
...
sub find_next_prime
{
	...
	my($num)=$prime_list[$#prime_list]+1;
	while(1) #Soooo bad !
	{
...
sub remove_non_primes
{
	...
	my($i,$m)=0;
	for($i=0;$i<=$#{$refprimes};$i++)
	{
		my($p)=$$refprimes[$i];
		my($k_start)=$start/$p;
		my($k_end)=int($end/$p);
		if($k_start != int($k_start))
		{
			$k_start = int($k_start)+1;
		}
		for($m=($k_start*$p)-$start;$m<=$end-$start;$m+=$p)
		{
			$$refcrible[$m]=0;
		}
	}
